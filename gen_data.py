...
def write_csv(data_file, save_dir):
    data_type = data_file.split("/")[-1]
    csv_file = os.path.join(save_dir, data_type+".csv")
    for class_file in os.listdir(data_file):
        class_dir = os.path.join(data_file, class_file)
        imgs = [os.path.join(class_dir, img) for img in os.listdir(class_dir)]
        lables = [int(class_file)] * len(imgs)
        with open(csv_file, mode='a', newline='') as file:
            writer = csv.writer(file)
            writer.writerows(zip(imgs, lables))
    print("the csv file is finished: {}".format(csv_file))
...
if __name__ == '__main__':
    data_dir = '/data/gaoshan/MNIST/'
    # parser_mnist_data(data_dir)

    write_csv('/data/gaoshan/MNIST/test', data_dir)